...
{
    assert(mutex_lock(&state[row][col].cell_mutex) >= 0);

    if (state[row][col].exiting) {
        assert(mutex_unlock(&state[row][col].cell_mutex) >= 0);
        return;
    }
...
int main(int argc, char *argv[])
{
    ...
    for (row = 0; row < CONSOLE_HEIGHT - 1; row++) {
        for (col = 0; col < CONSOLE_WIDTH; col++) {
            assert(mutex_destroy(&state[row][col].cell_mutex) >= 0);
        }
    }

    for (i = 0; i < num_traps; i++) {
        assert(cond_destroy(&traps[i]) >= 0);
    }

    assert(mutex_destroy(&console_lock) >= 0);
