...
void *race(void *arg)
{
    ...
    assert(mutex_lock(&lock_print));
    color += get_ticks();
    set_term_color(color);
    assert(set_cursor_pos(row, 0));
    printf("%s", &empty[0]);
    assert(mutex_unlock(&lock_print));

    while(1)
    {
        assert(sem_wait(&sem));
        if(++running > sems)
            lprintf("BAD! I have %d > %d threads running", running, sems);
        
        assert(mutex_lock(&lock_print));
        assert(set_cursor_pos(24, 77));
        printf("%.2d", running);
        assert(mutex_unlock(&lock_print));
...
int main( int argc, char *argv[] )
{
    ...
    assert(thr_init(STACK_SIZE));
    assert(mutex_init(&lock_print));
    assert(sem_init(&sem, sems));

    for(i=0; i<80; i++)
        empty[i] = ' ';
    empty[80] = '\0';
   
    assert(thr_create(bad, (void*)i) >= 0);
    
    for(i=0; i<threads; i++)
        assert(thr_create(race, (void*)i) >= 0);
    
    getchar();
    assert(set_cursor_pos(24,0));
