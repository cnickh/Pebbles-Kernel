...
void inc_count(void)
{
    if (mutex_lock(&count_mutex) != 0) {
        lprintf("Failed to lock count_mutex.\n");
        return;
    }
    th_count++;
    if (mutex_unlock(&count_mutex) != 0) {
        lprintf("Failed to unlock count_mutex.\n");
    }
    return;
}
    ...
    if (n > 0) {
        for (throws = 0; throws < n_throws; throws++) {
            ...
            if ((ret = thr_join(sub1, (void*)&substat))
              ...
              lprintf("ret = %d, %d ?= %d, %d ?= %d\n",
                                    ret, sub1, subid, (n - 1), substat);
            }
