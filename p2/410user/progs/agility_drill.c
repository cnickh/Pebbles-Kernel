 ...
 *
 *  Tests: Aggressively tests acquire/release of lots of mutexes with the occasional sleep or yield
 *  call in the middle.
 *
 *  Agility drill takes three arguments (all of which have default values - if you give only zero, one or
 *  two arguments the defaults are used for the un-set arguments).
 ...
 * the main loop after it releases a lock with index divisible by 4. This is
 * essentially idiotic behavior but it stops the kernel from just running each
 * thread to completion as soon as it's completed (that'd be too easy, now, wouldn't
 * it). The stupid magic constant also determines how many threads are required
 * (at a minimum) to make this test meaningful).
 *
 *  @author Geoff Langdale (geoffl)
 *
 *  @bug None known
 **/
...
// note: this test may not bother with status after all - it's too tedious to release all the locks on a failed
// thread, so any kind of failure will gum things up for good
...
void * chase(void * arg) {
  ...
  while (highest_mutex_released < n_mutexes - 1) {
    ...
    if (next_mutex_to_acquire < n_mutexes) {
      assert(mutex_lock(&(mtxs[next_mutex_to_acquire++])) == 0);
    }
    ...
    if (next_mutex_to_acquire >= footprint) {
      assert(mutex_unlock(&(mtxs[++highest_mutex_released])) == 0);
    }
  }
...
int main( int argc, char *argv[] ) {
    ...
    if ( (argc > 4) ||
        ...
        printf("Usage: %s [number of mutexes] [number of chase threads] [footprint]\n", argv[0]);
