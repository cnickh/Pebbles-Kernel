...
{
        if (mutex_lock(&big_global_mutex)) {
                lprintf("Oops! Mutexes broken!");
                exit(1);
        }

        if (cond_wait(&c, &big_global_mutex)) {
                lprintf("Oops! Condition variables broken!");
                exit(1);
        }
...
int main( int argc, char *argv[] ) 
{
  ...
  thrgrp_init_group(&maniac_grp);
	...
	for (i=0;i<NR_MANIACS;++i) {
		if ((maniac_tid=thrgrp_create(&maniac_grp, maniac, NULL)) < 0) {
			lprintf("Error in thread Creation!");
			exit(1);
		}

		thr_yield(maniac_tid);
	}
