...
AFS_PATH=${AFS_PATH-/afs/cs.cmu.edu/academic/class/15410-s07/Web/update/proj2}
WEB_URL=${WEB_URL-'http://www.cs.cmu.edu/~410-s07/update/proj2'}
...
function fetch () {
    ...
    case "$METHOD" in
	afs) cp "$AFS_PATH/$from" "$to" ;;
	web) wget -nv "$WEB_URL/$from" -O "$to" ;;
	query)
...
function do_update () {
    ...
    test -z "$NEEDS_UPDATING" || { echo -e '****\nUpdates are available. Please run make update sometime soon.\n****\7'; sleep 1; echo -e '\7'; sleep 1; echo -e '\7'; }
    return $rc
}
