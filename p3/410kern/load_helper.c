...
elf_load_helper( simple_elf_t *se_hdr, const char *fname )
{
    ...
    if( ret != sizeof( Elf32_Ehdr ) ) {
        lprintf_kern( "Loader: Couldn't read elf header: %d, %s", ret, fname );
        return ELF_NOTELF;
    }
    ...
    if( elf_hdr.e_shnum > 12 ) {
        lprintf_kern( "Loader: too many section headers" );
    } 
    */
    ...
    if( !elf_sec_hdrs ) {
        lprintf_kern( "Loader: not enough mem for sec_hdr array" );
        return ELF_NOTELF;
    }
    ...
    if( ret != size ) {
        lprintf_kern( "Loader: could not read section headers" );
        free( elf_sec_hdrs );
        return ELF_NOTELF;
    }
    ...
    if( !shstrtab ) {
        lprintf_kern( "Loader: not enought mem for sec hdr str tab" );
        free( elf_sec_hdrs );
        return ELF_NOTELF;
    }
    ...
    ret = getbytes( fname, elf_sec_hdrs[elf_hdr.e_shstrndx].sh_offset,
                    size, shstrtab );
    if( ret != size ) {
        lprintf_kern( "Loader: could not read sec hdr str tab" );
    ...
    for( i = 0; i < elf_hdr.e_shnum; i++ ) {
	  ...
	  lprintf_kern( "Loader: unknown header: \"%s\"",
			&(shstrtab[ str_idx ]));
	}
