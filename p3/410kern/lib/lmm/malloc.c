...
{
	void *ptr;
	ptr = malloc_flags(nelt*eltsize,0);
	if(ptr != NULL) {
		bzero(ptr, nelt*eltsize);
	}
	return ptr;
}
...
void _free(void *chunk)
{
        return free_flags(chunk);
}

void *_realloc(void *chunk, unsigned int new_size)
{
        unsigned int old_size;
        unsigned int *new_chunk;

	new_chunk = _malloc(new_size);
        if (new_chunk == 0) 
                return 0;

	if (chunk) {
                old_size = *((unsigned int *)chunk - 1);
