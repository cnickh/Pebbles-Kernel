 ...
 *
 * tests the basic functionality of yield() and cas_runflag()
 * yield_desc_mkrun.c
 */
...
int main()
{
  ...
  if (childtid == 0) {
    ...
    expect = -1;
    new = 1;
    if((error = cas_runflag(parenttid, &old, &expect, &new)) != 0) {
      lprintf("cas_runflag() failed %d in child", error);
      goto fail;
    }
    if(old != expect) {
    ...
    new = -1;
    if((error = cas_runflag(parenttid, &old, &expect, &new)) != 0) {
      lprintf("cas_runflag() failed %d in parent", error);
      goto fail;
    }
    if(old != expect) {
